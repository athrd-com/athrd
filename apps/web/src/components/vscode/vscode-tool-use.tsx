...
import type { ToolInvocationSerialized, MCPToolSource, TerminalToolData } from "@/types/vscode";
...
export default function VSCodeToolUse({ tool, result }: VSCodeToolUseProps) {
    ...
    if (tool.source?.type === "mcp") {
        ...
        const resultDetails = tool.resultDetails as any;

        return (
            <ToolMCPBlock
                serverName={mcpData.serverLabel || "MCP Server"}
                toolName={mcpData.label || tool.toolId}
                input={resultDetails?.input ? JSON.parse(resultDetails.input) : undefined}
                result={resultDetails?.output || result}
            />
        );
    }
    ...
    if (tool.toolId === "run_in_terminal") {
        ...
        if (
            ...
            command = (tool.toolSpecificData as TerminalToolData).commandLine.original || "";
        }
        return <ShellBlock command={command} result={typeof result === 'string' ? result : undefined} />;
    }
